public class Polynomial {
    ...
    public void print() {
        for (int i = 0; i < coefficients.size(); i++) {
            if (coefficients.get(i) != 0) {
                if (coefficients.get(i) < 0) {
                    System.out.print(" - " + Math.abs(coefficients.get(i)) + "x^" + i);
                } else {
    ...
    public void multiply(Polynomial p) {
        ...
        for (int i = 0; i <= p.degree(); i++) {
            for (int j = 0; j <= this.degree(); j++) {
                newPoly.setCoefficient(i + j,
                        newPoly.getCoefficient(i + j) + this.getCoefficient(j) * p.getCoefficient(i));
            }
        }
    ...
    public int evaluate(int x) {
        ...
        int ans = 0;
        while (i <= this.degree()) {
            ans = ans + currentDegree * this.getCoefficient(i);
